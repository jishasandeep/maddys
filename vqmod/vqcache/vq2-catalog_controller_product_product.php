...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['text_color_disclaimer'] = $this->language->get('text_color_disclaimer');
...
$data['quantity'] = $product_info['quantity'];
			...
			//JB
			if($data['special']){
				$data['freeship'] = ($this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'))) >= 500  ? true : false ;
			}else if ($data['price']) {
				$data['freeship'] = ($this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax')) )  >= 500  ? true : false ;
			} else {
				$data['freeship'] = false;
			}
			...
			$data['freeship_img'] = $server . 'image/maddy/freeship.png';


			//Start: SizeChart - JB
			$attr_groups = $this->model_catalog_product->getProductAttributes($this->request->get['product_id']);
			$data['sizeguide_type'] = '';
			 foreach ($attr_groups as $attribute_group) {
			 	 foreach ($attribute_group['attribute'] as $attribute) {
						if($attribute['name'] == 'Size Chart'){
							$data['sizeguide_type'] = $attribute['text'];

						} else 	if($attribute['name'] == 'Custom' && ( $attribute['text'] == 'Y' || $attribute['text'] == 'PreOrder')){
								$data['stock'] = 'Ready to Stitch';
			...
			$data['sizeguide_img'] = $server . 'image/maddy/sizeguide_';
